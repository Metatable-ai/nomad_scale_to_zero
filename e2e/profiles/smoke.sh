...
: "${E2E_PROFILE_DESCRIPTION:=Minimal smoke target for fast e2e validation with one fast-api, one slow-start, and one dependency-sensitive workload}"
: "${E2E_AUTOMATION_SMOKE_JOB:=e2e-smoke}"
: "${E2E_AUTOMATION_CERTIFICATION_JOB:=e2e-certification}"
: "${E2E_AUTOMATION_JOB_NAME:=e2e-smoke}"
...
: "${E2E_WORKLOAD_FAST_API_COUNT:=1}"
: "${E2E_WORKLOAD_SLOW_START_COUNT:=1}"
: "${E2E_WORKLOAD_DEPENDENCY_SENSITIVE_COUNT:=1}"
...
: "${E2E_SOAK_CYCLES:=1}"
: "${E2E_SCENARIO_SET:=mixed-traffic,dead-job-revival}"

: "${E2E_TRAFFIC_SHAPE:=warmup,rolling}"
: "${E2E_TRAFFIC_SCENARIO:=warmup}"
: "${E2E_K6_TARGET_MODE:=round-robin}"
: "${E2E_WARMUP_VUS:=2}"
: "${E2E_WARMUP_DURATION:=5s}"
: "${E2E_BURST_VUS:=10}"
: "${E2E_BURST_DURATION:=10s}"
: "${E2E_STORM_START_RATE:=5}"
: "${E2E_STORM_RATE:=25}"
: "${E2E_STORM_DURATION:=10s}"
: "${E2E_STORM_PREALLOCATED_VUS:=25}"
: "${E2E_STORM_MAX_VUS:=50}"

: "${E2E_GATE_TRAFFIC_SUCCESS_RATE:=0.95}"
: "${E2E_GATE_WAKE_P95_MS:=60000}"
: "${E2E_GATE_WAKE_P99_MS:=90000}"
: "${E2E_GATE_SCALE_TO_ZERO_MAX_SECONDS:=180}"
: "${E2E_GATE_DEPENDENCY_READY_MAX_SECONDS:=120}"
